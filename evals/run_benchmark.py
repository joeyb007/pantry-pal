...
def generate_with_local_model(model_path: str, prompts: list[dict]) -> list[str]:
    ...
    for i, p in enumerate(prompts):
        ...
        if (i + 1) % 50 == 0:
            print(f"  {i + 1}/{len(prompts)} completions done")
    return completions
...
def generate_with_gpt4o(prompts: list[dict]) -> list[str]:
    ...
    for i, p in enumerate(prompts):
        ...
        if (i + 1) % 50 == 0:
            print(f"  {i + 1}/{len(prompts)} GPT-4o completions done")
    return completions
...
def run_benchmark(
    ...
    print("Generating with base Llama...")
    base_completions = generate_with_local_model(base_model_path, prompts)
    ...
    base_csr = compute_csr(base_completions, prompts)
    ...
    print("Scoring quality (SFT)...")
    sft_quality = judge_quality(sft_completions, prompts)
    ...
    sft_delta = round(sft_csr["csr"] - base_csr["csr"], 2)
    dpo_delta = round(dpo_csr["csr"] - sft_csr["csr"], 2)
...
|-------|-----|------------|
| Base Llama 3.2 3B | {base_csr['csr']}% | {base_csr['violations']}/{base_csr['total']} |
| PantryPal SFT | {sft_csr['csr']}% | {sft_csr['violations']}/{sft_csr['total']} |
...
| Model | Score (1-10) |
|-------|-------------|
| PantryPal SFT | {sft_quality} |
| PantryPal DPO | {dpo_quality} |
| GPT-4o | {gpt4o_quality} |

## Training Delta

SFT vs base: {sft_sign}{sft_delta}pp ({base_csr['csr']}% → {sft_csr['csr']}%)
DPO vs SFT:  {dpo_sign}{dpo_delta}pp ({sft_csr['csr']}% → {dpo_csr['csr']}%)
...
"""
    per_restriction_rows = "".join(
        f"| {k} | {v}% |\n" for k, v in sorted(sft_csr["per_restriction"].items())
    )
...
if __name__ == "__main__":
    ...
    run_benchmark(args.dpo_model, args.sft_model, args.base_model)
